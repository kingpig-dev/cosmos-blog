...
func init() {
	...
	md_Loan = File_blog_blog_loan_proto.Messages().ByName("Loan")
...
func (x *Loan) slowProtoReflect() protoreflect.Message {
	mi := &file_blog_blog_loan_proto_msgTypes[0]
...
func (x *fastReflection_Loan) Has(fd protoreflect.FieldDescriptor) bool {
	switch fd.FullName() {
	case "blog.blog.Loan.id":
		return x.Id != uint64(0)
	case "blog.blog.Loan.amount":
		return x.Amount != ""
	case "blog.blog.Loan.fee":
		return x.Fee != ""
	case "blog.blog.Loan.collateral":
		return x.Collateral != ""
	case "blog.blog.Loan.deadline":
		return x.Deadline != ""
	case "blog.blog.Loan.state":
		return x.State != ""
	case "blog.blog.Loan.borrower":
		return x.Borrower != ""
	case "blog.blog.Loan.lender":
		return x.Lender != ""
	default:
		if fd.IsExtension() {
			panic(fmt.Errorf("proto3 declared messages do not support extensions: blog.blog.Loan"))
		}
		panic(fmt.Errorf("message blog.blog.Loan does not contain field %s", fd.FullName()))
	}
}
...
func (x *fastReflection_Loan) Get(descriptor protoreflect.FieldDescriptor) protoreflect.Value {
	...
	default:
		...
		panic(fmt.Errorf("message blog.blog.Loan does not contain field %s", descriptor.FullName()))
	}
}
...
func (x *fastReflection_Loan) Mutable(fd protoreflect.FieldDescriptor) protoreflect.Value {
	switch fd.FullName() {
	case "blog.blog.Loan.id":
		panic(fmt.Errorf("field id of message blog.blog.Loan is not mutable"))
	case "blog.blog.Loan.amount":
		panic(fmt.Errorf("field amount of message blog.blog.Loan is not mutable"))
	case "blog.blog.Loan.fee":
		panic(fmt.Errorf("field fee of message blog.blog.Loan is not mutable"))
	case "blog.blog.Loan.collateral":
		panic(fmt.Errorf("field collateral of message blog.blog.Loan is not mutable"))
	case "blog.blog.Loan.deadline":
		panic(fmt.Errorf("field deadline of message blog.blog.Loan is not mutable"))
	case "blog.blog.Loan.state":
		panic(fmt.Errorf("field state of message blog.blog.Loan is not mutable"))
	case "blog.blog.Loan.borrower":
		panic(fmt.Errorf("field borrower of message blog.blog.Loan is not mutable"))
	case "blog.blog.Loan.lender":
		panic(fmt.Errorf("field lender of message blog.blog.Loan is not mutable"))
	default:
		if fd.IsExtension() {
			panic(fmt.Errorf("proto3 declared messages do not support extensions: blog.blog.Loan"))
		}
...
func (x *fastReflection_Loan) WhichOneof(d protoreflect.OneofDescriptor) protoreflect.FieldDescriptor {
	switch d.FullName() {
	default:
		panic(fmt.Errorf("%s is not a oneof field in blog.blog.Loan", d.FullName()))
	}
	panic("unreachable")
}
...
func (*Loan) Descriptor() ([]byte, []int) {
	return file_blog_blog_loan_proto_rawDescGZIP(), []int{0}
}
...
var File_blog_blog_loan_proto protoreflect.FileDescriptor

var file_blog_blog_loan_proto_rawDesc = []byte{
	0x0a, 0x14, 0x62, 0x6c, 0x6f, 0x67, 0x2f, 0x62, 0x6c, 0x6f, 0x67, 0x2f, 0x6c, 0x6f, 0x61, 0x6e,
	0x2e, 0x70, 0x72, 0x6f, 0x74, 0x6f, 0x12, 0x09, 0x62, 0x6c, 0x6f, 0x67, 0x2e, 0x62, 0x6c, 0x6f,
	0x67, 0x22, 0xc6, 0x01, 0x0a, 0x04, 0x4c, 0x6f, 0x61, 0x6e, 0x12, 0x0e, 0x0a, 0x02, 0x69, 0x64,
	...
	0x6d, 0x2e, 0x62, 0x6c, 0x6f, 0x67, 0x2e, 0x62, 0x6c, 0x6f, 0x67, 0x42, 0x09, 0x4c, 0x6f, 0x61,
...
var (
	file_blog_blog_loan_proto_rawDescOnce sync.Once
	file_blog_blog_loan_proto_rawDescData = file_blog_blog_loan_proto_rawDesc
)

func file_blog_blog_loan_proto_rawDescGZIP() []byte {
	file_blog_blog_loan_proto_rawDescOnce.Do(func() {
		file_blog_blog_loan_proto_rawDescData = protoimpl.X.CompressGZIP(file_blog_blog_loan_proto_rawDescData)
	})
	return file_blog_blog_loan_proto_rawDescData
}

var file_blog_blog_loan_proto_msgTypes = make([]protoimpl.MessageInfo, 1)
var file_blog_blog_loan_proto_goTypes = []interface{}{
	(*Loan)(nil), // 0: blog.blog.Loan
}
var file_blog_blog_loan_proto_depIdxs = []int32{
...
func init() { file_blog_blog_loan_proto_init() }
func file_blog_blog_loan_proto_init() {
	if File_blog_blog_loan_proto != nil {
		return
	}
	if !protoimpl.UnsafeEnabled {
		file_blog_blog_loan_proto_msgTypes[0].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*Loan); i {
			case 0:
				return &v.state
			case 1:
	...
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			...
			RawDescriptor: file_blog_blog_loan_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   1,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_blog_blog_loan_proto_goTypes,
		DependencyIndexes: file_blog_blog_loan_proto_depIdxs,
		MessageInfos:      file_blog_blog_loan_proto_msgTypes,
	}.Build()
	File_blog_blog_loan_proto = out.File
	file_blog_blog_loan_proto_rawDesc = nil
	file_blog_blog_loan_proto_goTypes = nil
	file_blog_blog_loan_proto_depIdxs = nil
}
